...
- **hook 命令自定位引导（四方言同串）**：每事件四个方言键（`command`/`bash`/`windows`/`powershell`）是同一条 `node -e "<bootstrap>" <subcommand>` 命令。引导在 Node 进程内按序解析插件根：env `PLUGIN_ROOT`/`COPILOT_PLUGIN_ROOT`/`CLAUDE_PLUGIN_ROOT`（逐一 existsSync 校验）→ `process.cwd()`（CLI 执行插件 hook 时 cwd=插件目录）→ `~/.copilot/installed-plugins/openviking-team/openviking-copilot`（CLI plugin reference 文档化的稳定安装路径），命中即以 `stdio:'inherit'` spawn `scripts/hook-runner.mjs` 并透传退出码与 stdin，全部未命中静默退出 0。根因链：`${PLUGIN_ROOT}` 在 PowerShell 是 **PS 变量**语法而非环境变量引用，宿主选 `command` 键经 PS 执行时无论 env 是否注入都插值为空 → `node "/scripts/hook-runner.mjs"` → 当前盘符根（`E:\scripts\hook-runner.mjs`）；`$env:PLUGIN_ROOT`（0.4.5 的 windows/powershell 方言）依赖宿主注入 env 且依赖宿主选择方言键，旧版 CLI 两条都不满足。因此引导串刻意不含 `$`、反引号、嵌套双引号——PowerShell、bash、cmd 均原样传给 node。超时字段必须是 `timeoutSec`——CLI 对含未知字段（如 `timeout`）的 hook 条目整体静默丢弃。`plugin.test.mjs` 有三重断言：命令串与规范模板全等、无 shell 插值字符、env/cwd/固定路径三级解析与退出码/stdin 透传行为
